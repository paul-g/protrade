...
public class PredictionCalculatorTest {
    ...
    public void GamePrediction(){
    	...
    	double[] TbGameExpMatrix = {
				0.53, 0.44, 0.29, 0.20, 0.10, 0.04, 0.01, 0,
				0.67, 0.53, 0.43, 0.27, 0.17, 0.07, 0.02, 0,            
				0.76, 0.68, 0.53, 0.42, 0.24, 0.13, 0.03, 0,
				0.87, 0.77, 0.69, 0.53, 0.40, 0.20, 0.08, 0,
				0.93, 0.89, 0.80, 0.72, 0.52, 0.37, 0.13, 0,
				0.98, 0.95, 0.92, 0.83, 0.75, 0.52, 0.32, 0,
				0.99, 0.99, 0.98, 0.96, 0.89, 0.82, 0.52, -1,
				1,    1,    1,    1,    1,    1,    -1,   -1
    		};
    	// second player serves in tiebreak
    	double[] TbGameExpMatrixSnd = {
    			0.53, 0.39, 0.29, 0.17, 0.10, 0.03, 0.01, 0,
    			0.62, 0.53, 0.37, 0.27, 0.14, 0.07, 0.01, 0,
    			0.76, 0.63, 0.53, 0.35, 0.24, 0.10, 0.03, 0,
    			0.83, 0.77, 0.63, 0.53, 0.33, 0.20, 0.05, 0,
    			0.93, 0.86, 0.80, 0.65, 0.52, 0.29, 0.13, 0,
    			0.97, 0.95, 0.89, 0.83, 0.67, 0.52, 0.21, 0,
    			0.99, 0.99, 0.98, 0.93, 0.89, 0.71, 0.52, 2,
    			1,    1,    1,    1,    1,    1,	2,    2
    		};
    	...
    	try{
	    	...
	    	Method method = PredictionCalculator.class.getDeclaredMethod("calculateGamePercent", args1);
	    	...
	    	for (int i=0; i<5; i++){
	    		for (int j=0; j<5; j++){
	    			...
	    			//System.out.println("RES"+res + "EXP"+GameExpMatrix[i*5+j]);
	    			assertEquals(0, Double.compare(res, GameExpMatrix[i*5+j]) );
	    		}
	    	} 
	    	
	    	Object[] value = {new Integer(0), new Integer(1), new Double(0.60), new Integer(1) };
			Double res2 = Double.parseDouble(df.format((Double)method.invoke(calc,(Object[])value)));
	    	...
	    	Method method2 = PredictionCalculator.class.getDeclaredMethod("calculateTiebreakerGamePercent", args2);
	    	
	    	//The conditional probabilities of player A winning the tiebreaker game
	    	//from various score lines for pA = 0.62 and pB = 0.60, and player A serving
	    	method.setAccessible(true);
	    	for (int i=0; i<8; i++){
	    		for (int j=0; j<8; j++){
	    	...
	    	//The conditional probabilities of player A winning the tiebreaker game
	    	//from various score lines for pA = 0.62 and pB = 0.60, and player B serving
	    	for (int i=0; i<8; i++){
	    		for (int j=0; j<8; j++){
     ...
     public void SetPrediction(){
    	 double[] expMatrix = {
    		        0.456, 0.232, 0.178, 0.05,  0.026, 0.002,   0,
    		        0.518, 0.459, 0.213, 0.153, 0.032, 0.011,   0,
    		        0.764, 0.528, 0.463, 0.187, 0.12,  0.014,   0,
    		        0.83,  0.799, 0.54,  0.466, 0.149, 0.072,   0,
    		        0.962, 0.871, 0.844, 0.555, 0.47,  0.092,   0,
    		        0.987, 0.984, 0.925, 0.907, 0.576, 0.473, 0.093,
    		        1,     1,     1,     1,     1,     0.58,  0.478
				 };
    	DecimalFormat df = new DecimalFormat("#.###");
		...
		try{
			...
			args[3] = Double.TYPE;
			args[4] = Integer.TYPE;
			Method method = PredictionCalculator.class.getDeclaredMethod("calculateSetPercent", args);
			method.setAccessible(true);
			// Player 1 starts the set, therefore at odd scores i+j he is serving
			for (int i=0; i<7; i++){
				for (int j=0; j<7; j++){
					Object[] values = {new Integer(i), new Integer(j), new Double(0.6227), new Double(0.6359), new Integer(1- (i+j)%2)};
					Double res = Double.parseDouble(df.format((Double)method.invoke(calc,(Object[])values)));
					//System.out.println("At score "+i+"-"+j+ " RES ="+res + " EXP="+expMatrix[i*7+j]);			
     ...
     public void AdvSetPrediction(){
    	double[] expMatrix = {
                0.57, 0.50, 0.27, 0.19, 0.05, 0.02, 0,
                0.77, 0.57, 0.49, 0.23, 0.15, 0.02, 0,
                0.83, 0.78, 0.56, 0.47, 0.19, 0.09, 0,
                0.94, 0.85, 0.81, 0.56, 0.46, 0.11, 0,
                0.97, 0.96, 0.88, 0.84, 0.55, 0.43, 0,
                1.00, 0.99, 0.98, 0.93, 0.90, 0.55, -1,
                1,    1,    1,    1,    1,	  -1,   -1
		};
    	DecimalFormat df = new DecimalFormat("#.##");
		...
		try{
			...
			args[3] = Double.TYPE;
			args[4] = Integer.TYPE;
			Method method = PredictionCalculator.class.getDeclaredMethod("calculateAdvSetPercent", args);
			method.setAccessible(true);
			// Player 1 starts the set, therefore at odd scores i+j he is serving
			for (int i=0; i<7; i++){
				for (int j=0; j<7; j++){
     ...
     public void MatchPrediction(){
		...
		try{
			...
			args[4] = Integer.TYPE;
			args[5] = Integer.TYPE;
			Method method = PredictionCalculator.class.getDeclaredMethod("calculateMatchPercent", args);
			method.setAccessible(true);
			for (int i=0; i<4; i++){
				for (int j=0; j<4; j++){
					Object[] values = {new Integer(i), new Integer(j), new Double(0.62), new Double(0.60), 5, 1};
					Double res = Double.parseDouble(df.format((Double)method.invoke(calc,(Object[])values)));
					//System.out.println("At score "+i+"-"+j+ " RES ="+res + " EXP="+expMatrix[i*4+j]);			
     ...
     public void allPrediction(){
		 // Test cases from GUI in Wozniak - player 2 serving, 00-00(points), 4-4(games), 1-0(sets),
		 double pw1 = 0.6527;
		 double pw2 = 0.6452;
		 int p1 = 0;
		 int p2 = 0;
		 ...
		 double winCurGamePlayer1 = Double.parseDouble(df.format(1-winCurGamePlayer2));
		 ...
		 double winCurSetPlayer2 = Double.parseDouble(df.format(calc.calculateCurrentSetPercent(4, 4, pw1, pw2, 0)));
		 double winCurSetPlayer1 = Double.parseDouble(df.format(1-winCurSetPlayer2));
		 ...
		 double winCurMatchPlayer2 = Double.parseDouble(df.format(calc.calculateWinMatch(winCurSetPlayer2, 1, 0, pw1, pw2, 0, 5, 1)));
		 double winCurMatchPlayer1 = Double.parseDouble(df.format(1-winCurMatchPlayer2));
