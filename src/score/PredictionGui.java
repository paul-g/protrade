...
public class PredictionGui {
  ...
  private void createScoreContents(Composite composite, String matchName) {
   ...
   // toolBar.setBounds(new Rectangle(331, 8, 100, 30));
    ...
    String name2 = matchName.substring(matchName.indexOf("v ") + 2, matchName
        .length());
    if (name1.contains("/"))
    ...
    int c = 0;
    if (matchIndex >= 8
        && scores.substring(matchIndex - 8, matchIndex - 2).compareTo("SERVER") == 0) {
...
*/
  ...
  private String extractScores(String match) throws Exception {
     ...
     * HtmlPage page = webClient.getPage(request); // try 20 times to wait .5
     * second each for filling the page. for (int i = 0; i < 2; i++) { //page
     * scores = page.getElementById("allzapasy");
     * 
    ...
    Iterator iter = (scores.getElementsByAttribute("img", "src",
        "/img/tennisball.gif")).iterator();
    while (iter.hasNext()) {
