...
public class StatisticsParser extends SiteParser{
	 ...
	 public void parseAndSetStatistics() {
	        ...
	        // Matches, sets, games
	        for (int i = 0; i < 3; i++) {
	            getSkipUntil(playerOneWonLost, playerTwoWonLost, statistic++, ")", ")", "W/L", 3);
	        }
		...
		private void getSkipUntil(String[] playerOneWonLost,
				String[] playerTwoWonLost, int i, String getUntilToken, String skipBeforeToken, String internalToken, int inc) {
			playerOneWonLost[i] = stats.substring(0, stats.indexOf(getUntilToken) + 1);
			stats = stats.substring(stats.indexOf(internalToken) + inc, stats.length());
			stats = skipEmptyLines(stats);
			playerTwoWonLost[i] = stats.substring(0, stats.indexOf(skipBeforeToken) + 1);
			stats = skipLines(stats, 1);
		}
