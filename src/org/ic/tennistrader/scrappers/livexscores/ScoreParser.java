...
public class ScoreParser extends SiteParser {
	...
	public void parseAndSetScores() {
		...
		int matchStart = scoreString.indexOf(match.getPlayerOne().getLastname()
				+ " " + match.getPlayerOne().getFirstname());

		PlayerEnum server = PlayerEnum.PLAYER1;
		// display server
		if (matchStart >= 8
				&& scoreString.substring(matchStart - 8, matchStart - 2)
						.compareTo("SERVER") == 0) {
			// player 1 serves
		...
		// String player1 = scoreString.substring(0, scoreString.indexOf(")") +
		// 1);
		...
		if (!scoreString.startsWith(match.getPlayerTwo().getLastname() + " "
				+ match.getPlayerTwo().getFirstname())) {
			// //////////////Player 1 data
			// Skip odds
			...
			// for 5 sets max
			for (int i = 0; i < 5; i++) {
				playerOneGames[i] = Integer.parseInt(scoreString.substring(0,
						scoreString.indexOf("\t")));
				scoreString = scoreString.substring(
						scoreString.indexOf("\t") + 1, scoreString.length());
			...
			if (!scoreString.startsWith(match.getPlayerTwo().getLastname())) {
				if (scoreString.substring(0, 2).equals("Ad"))
					...
					playerOnePoints = Integer.parseInt(scoreString.substring(0,
							2));
		...
		for (int i = 0; i < 5; i++) {
			try {
				playerTwoGames[i] = Integer.parseInt(scoreString.substring(0,
						scoreString.indexOf("\t")));
				scoreString = scoreString.substring(
						scoreString.indexOf("\t") + 1, scoreString.length());
