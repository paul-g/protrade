...
class WidgetContainerResizeListener implements Listener {
	...
	public void handleEvent(Event e) {
		...
		case SWT.MouseDown:
			if (e.button == 1){
			origin = widgetContainer.getDisplay().map(widgetContainer, null, x, y);
		...
		case SWT.MouseMove:
			if ( (e.stateMask & SWT.BUTTON3) != SWT.BUTTON3)
				this.widgetContainer.setCursor(widgetContainer.getCursorForLocation(x, y));
			if (origin != null && (e.stateMask & SWT.BUTTON1) == SWT.BUTTON1) { 
				Point eP = widgetContainer.getDisplay().map(widgetContainer, null, e.x, e.y);
				int dx = (eP.x - origin.x);
				int dy = (eP.y - origin.y);
				origin = eP;
				widgetContainer.getDashboard().handleResize(widgetContainer, dx, dy, initialLocation);
			}
			break;
		}
	}
