...
public class StatisticsUpdateThread extends MatchUpdaterThread {
	...
	private final Statistics playerOneStats;

	private final Statistics playerTwoStats;
	...
	private String getStatistics() throws Exception {
		...
		HtmlElement login = pageLogin.getElementById("LOGIN")
				.getElementsByTagName("form").get(0);
		HtmlTextInput name = (HtmlTextInput) login
				.getElementsByTagName("input").get(0);
		HtmlPasswordInput pass = (HtmlPasswordInput) login
				.getElementsByTagName("input").get(1);
		HtmlElement submitButton = login.getElementsByTagName("span").get(0)
				.getElementsByTagName("input").get(0);
		...
		HtmlElement body = loggedPage.getElementsByTagName("body").get(0);
		HtmlElement submitButton2 = body.getElementsByAttribute("td",
				"onclick", "match_preview_form_top_right.submit()").get(0);
		...
		HtmlElement btnContinue = intermPage.getElementById("addinsight");
		...
		if (retries > RETRY_LIMIT) {
			...
			try {
				...
				if (page != null) {
					...
					(new StatisticsParser(page, this.match))
							.parseAndSetStatistics();
