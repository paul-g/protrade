...
public class ScoreInference {
	 ...
	 * 
	 * Method must wait 7s for odds to stabilize and after wait for new market
	 * data; related to Heuristic 4: Averaging Odds During Fluctuation / page 46
	 * *Huang paper
	 * 
	...
	public int isPointScored(Match match, int avgPeriod) {
		...
		// wait 7s(avgPeriod)
		// Can you please help me on this, Corina? I'd need a separate thread
		// that runs this method
		// and put it to sleep for 7s

		double[] minMax = getOddsBounds(match, avgPeriod);
		double largeChange = StatisticMeasures
				.InterQuartileRange(PredictionCalculator.getOddsInTimePeriod(
						match, avgPeriod));
		double startOdds = match.getLastMarketData().getLastPriceMatched(
				match.getServer());

		// Keep checking odds till a point is score by either player
		while (change == -1 && threshold == -1 && changeOverManyPoints == -1) {
			double prevOdds = match.getLastMarketData().getLastPriceMatched(
					match.getServer());

			// wait for new MOdds
			// thread sleep till new MOdds arrive
			// Can you please help me on this, Corina ?
			// After separate thread is done, Shahin can plot point events on
			// chart

			double actualOdds = match.getLastMarketData().getLastPriceMatched(
					match.getServer());

			threshold = checkPointByThreshold(actualOdds, minMax[0], minMax[1],
					match.getServer());
			change = checkPointByLargeChange(actualOdds, prevOdds, largeChange,
					minMax[0], minMax[1], match.getServer());
			changeOverManyPoints = checkPointByLargeChange(actualOdds,
					startOdds, largeChange, minMax[0], minMax[1],
					match.getServer());
		}
		// Threshold check takes priority. If threshold values not attaines,
		// check for major odds change
	...
	/*
	 * Check if threshold min/max are exceeded by new market odds Returns 0 if
	 * player 1 won the point, 1 if player 2 scored, and -1 if neither scored
	 */
	private int checkPointByThreshold(double actualOdds, double min,
			double max, PlayerEnum server) {
		if (actualOdds >= max) {
	...
	/*
	 * Check if large change has occured with new market odds singly or over
	 * multiple points Returns 0 if player 1 won the point, 1 if player 2
	 * scored, and -1 if neither scored
	 */
	public int checkPointByLargeChange(double actualOdds, double prevOdds,
			double largeChange, double min, double max, PlayerEnum server) {
	...
	// Look at fluctuations of odds for the current server as these are likely
	// to fluctuate more when a point is scored
