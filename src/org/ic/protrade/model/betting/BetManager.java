...
import org.ic.protrade.ui.toolbars.BetsTable;
...
public class BetManager {
	...
	public static void placeBet(Match match, PlayerEnum player,
			BetTypeEnum betType, double odds, double amount) {
		...
		double firstWinnerProfit = getBetProfit(newBet,
				isBetSuccessful(newBet, PlayerEnum.PLAYER1));
		newBet.setFirstPlayerWinnerProfit(firstWinnerProfit);
		double secondWinnerProfit = getBetProfit(newBet,
				isBetSuccessful(newBet, PlayerEnum.PLAYER2));
	...
	private static void updatePossibleProfits(Bet newBet) {
		if (matchMarketData.containsKey(newBet.getMatch())) {
			VirtualBetMarketInfo marketInfo = matchMarketData.get(newBet
					.getMatch());
			marketInfo.addFirstPlayerWinnerProfit(newBet
					.getFirstPlayerWinnerProfit());
			marketInfo.addSecondPlayerWinnerProfit(newBet
		...
		if (matchMarketData.containsKey(bet.getMatch())) {
			...
			if (bet.getType().equals(BetTypeEnum.B)) {
				if (bet.getPlayer().equals(PlayerEnum.PLAYER1)) {
					virtualMarketInfo.addMatchedBackBetsFirstPlayer(
							bet.getOdds(),
							bet.getAmount() - bet.getUnmatchedValue());
				} else {
					virtualMarketInfo.addMatchedBackBetsSecondPlayer(
							bet.getOdds(),
							bet.getAmount() - bet.getUnmatchedValue());
				}
			} else {
				if (bet.getPlayer().equals(PlayerEnum.PLAYER1)) {
					virtualMarketInfo.addMatchedLayBetsFirstPlayer(
							bet.getOdds(),
							bet.getAmount() - bet.getUnmatchedValue());
				} else {
					virtualMarketInfo.addMatchedLayBetsSecondPlayer(
							bet.getOdds(),
							bet.getAmount() - bet.getUnmatchedValue());
				}
			}
	...
	public static void updateMarketAvailableMatches(Match match,
		...
		addNewMarketValues(
				virtualMarketInfo.getBackFirstPlayerAvailableMatches(),
				newMarketData.getPl1Back());
		addNewMarketValues(
				virtualMarketInfo.getLayFirstPlayerAvailableMatches(),
				newMarketData.getPl1Lay());
		addNewMarketValues(
				virtualMarketInfo.getBackSecondPlayerAvailableMatches(),
				newMarketData.getPl2Back());
		addNewMarketValues(
				virtualMarketInfo.getLaySecondPlayerAvailableMatches(),
				newMarketData.getPl2Lay());
	...
	private static void matchBet(VirtualBetMarketInfo virtualMarketInfo,
		...
		if (bet.getType().equals(BetTypeEnum.B)) {
			if (bet.getPlayer().equals(PlayerEnum.PLAYER1)) {
				virtualMarketInfo.addMatchedBackBetsFirstPlayer(bet.getOdds(),
						toMatch);
			} else {
				virtualMarketInfo.addMatchedBackBetsSecondPlayer(bet.getOdds(),
						toMatch);
			}
		} else {
			if (bet.getPlayer().equals(PlayerEnum.PLAYER1)) {
				virtualMarketInfo.addMatchedLayBetsFirstPlayer(bet.getOdds(),
						toMatch);
			} else {
				virtualMarketInfo.addMatchedLayBetsSecondPlayer(bet.getOdds(),
						toMatch);
			}
		}
	}

	private static void addNewMarketValues(
			HashMap<Double, Double> availableMatches,
	...
	public static void setBetsOutcome(Match match) {
		...
		try {
			...
			for (Bet bet : matchedBets) {
				if (bet.getMatch().equals(match)) {
					bet.setProfit(getBetProfit(bet,
							isBetSuccessful(bet, winner)));
					bt.refresh();
				}
			}
	...
	public static boolean isBetSuccessful(Bet bet, PlayerEnum winner) {
		if (winner == bet.getPlayer())
			if (bet.getType().equals(BetTypeEnum.B))
				...
				return false;
		else if (bet.getType().equals(BetTypeEnum.B))
			return false;
		else
			return true;
	}
	...
	public static double getFirstPlayerWinnerProfit(Match match) {
		if (matchMarketData.containsKey(match))
			return Double.valueOf(twoDForm.format(matchMarketData.get(match)
					.getFirstPlayerWinnerProfit()));
		else
			return 0.0;
	}
	...
	public static double getTotalLiabiltiy() {
		...
		for (Match match : matchMarketData.keySet()) {
			// System.out.println("Adding liability for " + match.toString() +
			// " which is " + matchMarketData.get(match).getLiability());
