...
public class OverroundChart extends UpdatableChart {
	...
	/*
	 * Old ones private void createSeries(String pl1Name, String pl2Name) {
	 * ISeriesSet seriesSet = this.getSeriesSet(); backOverround = (ILineSeries)
	 * seriesSet.createSeries(SeriesType.LINE, "back overround"); Color color =
	 * Display.getCurrent().getSystemColor(SWT.COLOR_BLUE);
	 * backOverround.setLineColor(color); layOverround = (ILineSeries)
	 * seriesSet.createSeries(SeriesType.LINE, "lay overround"); color =
	 * Display.getCurrent().getSystemColor(SWT.COLOR_RED);
	 * layOverround.setLineColor(color);
	 * 
	 * pl1Volume = (IBarSeries) seriesSet.createSeries(SeriesType.BAR, pl1Name +
	 * " Volume"); color = Display.getCurrent().getSystemColor(SWT.COLOR_BLUE);
	 * pl1Volume.setBarColor(color); pl1Volume.setBarPadding(90);
	 * 
	 * pl2Volume = (IBarSeries) seriesSet.createSeries(SeriesType.BAR, pl2Name +
	 * " Volume"); color = Display.getCurrent().getSystemColor(SWT.COLOR_RED);
	 * pl2Volume.setBarColor(color); pl2Volume.setBarPadding(90);
	 * backOverround.setVisible(false); layOverround.setVisible(false); //
	 * pl1Volume.setVisible(false); pl2Volume.setVisible(false); }
	 */
	...
	/*
	 * private void makeMenu(final OddsChart oddsChart, Composite parent) { Menu
	 * menu = new Menu(parent.getShell(), SWT.POP_UP); setMenu(menu);
	 * getPlotArea().setMenu(menu); final MenuItem overroundBack = new
	 * MenuItem(menu, SWT.CHECK); overroundBack.setText("Overround Back");
	 * overroundBack.setSelection(true); final MenuItem overroundLay = new
	 * MenuItem(menu, SWT.CHECK); overroundLay.setText("Overround Lay");
	 * overroundLay.setSelection(false); final MenuItem volume = new
	 * MenuItem(menu, SWT.CHECK); volume.setText("Volume");
	 * 
	 * overroundBack.addListener(SWT.Selection, new Listener() {
	 * 
	 * @Override public void handleEvent(Event arg0) { if
	 * (!overroundBack.getSelection() && !overroundLay.getSelection()) {
	 * overroundBack.setSelection(true); } else { volume.setSelection(false); }
	 * setBackOverround(overroundBack.getSelection());
	 * visibility(oddsChart.pl1Selected, oddsChart.pl2Selected); } });
	 * 
	 * overroundLay.addListener(SWT.Selection, new Listener() {
	 * 
	 * @Override public void handleEvent(Event arg0) { if
	 * (!overroundLay.getSelection() && !overroundBack.getSelection()) {
	 * overroundLay.setSelection(true); } else { volume.setSelection(false); }
	 ...
	 * 
	 * @Override public void handleEvent(Event arg0) { if
	 * (!volume.getSelection()) { volume.setSelection(true); } else {
	 * overroundBack.setSelection(false); overroundLay.setSelection(false);
	 * setBackOverround(false); setLayOverround(false);
	 * visibility(oddsChart.pl1Selected, oddsChart.pl2Selected); //
	 * allignment(); } } }); }
	 */

	/*
	 * public void showSeries(int i, boolean dragged) { int size = i + 1 <
	 * sampleSize ? i + 1 : sampleSize; int seriesNr = 4; Date showXSeries[] =
	 * new Date[size]; ArrayList<double[]> dataArray = new
	 * ArrayList<double[]>(); for (int k = 0; k < seriesNr; k++) {
	 ...
	 * 
	 * dataArray.get(nr)[a] = chartData.getBackOverround().get(b); nr++;
	 * dataArray.get(nr)[a] = chartData.getLayOverround().get(b); nr++;
	 ...
	 * setLayOverround(showXSeries, dataArray.get(1)); setPl1Volume(showXSeries,
	 * dataArray.get(2)); setPl2Volume(showXSeries, dataArray.get(3)); adjust();
	 * updateDisplay(); } }
	 */
}
