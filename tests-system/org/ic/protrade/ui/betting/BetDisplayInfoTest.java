...
public class BetDisplayInfoTest extends DisplayTest {
	...
	public void oneBet() {
		...
		double firstWinnerProfit = BetManager.getBetProfit(bet,
				BetManager.isBetSuccessful(bet, PlayerEnum.PLAYER1));
		double secondWinnerProfit = BetManager.getBetProfit(bet,
				BetManager.isBetSuccessful(bet, PlayerEnum.PLAYER2));
		betDisplayInfo.setPlayerWinnerProfits(firstWinnerProfit,
				secondWinnerProfit);
		assertEquals(betDisplayInfo.getFirstPlayerWinnerProfit(),
				firstWinnerProfit, Math.pow(10, -5));
		assertEquals(betDisplayInfo.getSecondPlayerWinnerProfit(),
				secondWinnerProfit, Math.pow(10, -5));
	}
